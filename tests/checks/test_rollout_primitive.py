...
def test_iter_rollout_returns_obs0_info0_and_iterator() -> None:
    """iter_rollout returns a 3-tuple of (ndarray, dict, iterator)."""
    result = iter_rollout(_SPEC_5X5, seed=42, action_policy=ActionPolicyId.SEEDED_RANDOM, n_steps=10)
    obs0, info_0, it = result
...
def test_obs0_is_start_position() -> None:
    ...
    obs0, info_0, it = iter_rollout(_SPEC_5X5, seed=42, action_policy=ActionPolicyId.SEEDED_RANDOM, n_steps=10)
    list(it)  # exhaust to close env
...
def test_info0_contains_grid_size() -> None:
    """Reset info dict contains grid_size per gridworld's reset implementation."""
    obs0, info_0, it = iter_rollout(_SPEC_5X5, seed=42, action_policy=ActionPolicyId.SEEDED_RANDOM, n_steps=5)
    list(it)  # exhaust to close env
    _ = obs0
    assert info_0.get("grid_size") == 5
...
def test_step_records_have_correct_types() -> None:
    ...
    for record in it:
        ...
        assert isinstance(record.info, Mapping)


def test_iter_rollout_yields_info_per_step() -> None:
    """Each StepRecord carries an info dict; gridworld steps return empty info."""
    spec = EnvSpec(
        entry_point="praxis.envs.gridworld:PraxisGridworld",
        kwargs={"grid_size": 5},
        max_episode_steps=100,
    )
    obs0, info_0, it = iter_rollout(spec, 42, ActionPolicyId.SEEDED_RANDOM, n_steps=5)
    assert isinstance(info_0, dict)
    assert info_0.get("grid_size") == 5
    records = list(it)
    for r in records:
        assert isinstance(r.info, Mapping)


def test_iterator_yields_up_to_n_steps() -> None:
    """With n_steps=10 and no early episode end, iterator yields at most 10 records."""
    ...
    obs0, info_0, it = iter_rollout(spec, seed=42, action_policy=ActionPolicyId.SEEDED_RANDOM, n_steps=10)
    _ = obs0
    _ = info_0
    records = list(it)
...
def test_iterator_stops_at_truncation() -> None:
    ...
    """
    obs0, info_0, it = iter_rollout(_SPEC_5X5, seed=42, action_policy=ActionPolicyId.SEEDED_RANDOM, n_steps=1000)
    _ = obs0
    _ = info_0
    records = list(it)
...
def test_step_record_is_frozen() -> None:
    ...
    obs0, info_0, it = iter_rollout(_SPEC_5X5, seed=42, action_policy=ActionPolicyId.SEEDED_RANDOM, n_steps=1)
    _ = obs0
    _ = info_0
    records = list(it)
